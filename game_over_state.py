...
class GameOverState:
    ...
    def __init__(self, gdata):
        self.gdata = gdata
        height = 220
        top = settings.WIN_CENTER[1] - int(height / 2)
        rect = pg.Rect(-10, top - 160, settings.WIN_SIZE[0] + 20, height)

        btn_size = 80, 50
        btn_left = int((settings.WIN_SIZE[0] - btn_size[0]) / 2)
        btn_top = rect.top + int(rect.height / 2)
        ...
        self.text = self.gdata.fonts["Large"].render("Game Over, You Lasted " +\
            str(int(self.gdata.time / 1000)) + " Seconds", True, settings.WHITE)
        text_left = int((settings.WIN_SIZE[0] - self.text.get_width()) / 2)
        text_top = int(rect.height / 4) - int(self.text.get_height() / 2) + rect.top
    ...
    def start_up(self):
        self.gdata.score.add(int(self.gdata.time / 1000))
